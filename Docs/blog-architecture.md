...
Status: approved and implemented through prerender + SEO.
...
Updated on 2026-03-07 after phase 4 completion:
...
- Angular now builds in `outputMode: static`.
- `/`, `/blog`, and `/blog/:slug` are prerendered during `ng build`.
- `rss.xml` and `sitemap-blog.xml` are generated by `build:content` into `public/`.
- Route-level SEO metadata is set from Angular and is present in the prerendered HTML.
- Blog and project JSON artifacts are read directly from generated files during SSR/prerender, while the browser keeps using `/assets/...`.
...
- static SEO artifacts (`rss.xml`, `sitemap-blog.xml`)
...
Purpose: blog feed generated during `build:content` and copied into the static output.

Rules:

- Includes published posts only.
- Uses canonical blog URLs.
- Uses `date` as `pubDate`.

### `public/sitemap-blog.xml`

Purpose: sitemap fragment for blog discovery generated during `build:content`.

Rules:

- Includes `/blog` and every published `/blog/<slug>` route.
- Uses `updatedAt` when available, otherwise `date`.
...
6. Adjust Firebase deploy rules for static prerender output.
